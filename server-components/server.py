...
async def _heavy_init() -> None:
    ...
    global world_engine, inpainting_manager, safety_checker, safety_hash_cache, startup_complete, startup_error, _is_mlx
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        try:
            if world_engine.platform == "mlx":
                import subprocess
                _metrics_gpu_name = subprocess.check_output(
                    ["sysctl", "-n", "machdep.cpu.brand_string"], text=True
                ).strip() + " (Apple GPU)"
            elif torch.cuda.is_available():
                _metrics_gpu_name = torch.cuda.get_device_name(0)
            else:
